 ...
 */
export async function conMensajesDeCobertura<T>(llamada: () => Promise<T>): Promise<T> {
  try {
    return await llamada();
  } catch (error) {
    const traducido = mensajeDeCobertura(error);
