...
export function ExecutiveDashboard() {
  const load = useCallback(async (): Promise<DashboardData> => {
    /*
     * Con Publicidad oculta ni siquiera se PIDE su tablero: una petición a un módulo que no se
     * enseña sólo puede producir un «sin respuesta de: Ads» sobre una función que nadie ve.
     */
    const [accounts, partners, glAccounts, audit, ads] = await Promise.all([
      ...
      PUBLICIDAD_VISIBLE
        ? capture('Publicidad', adsService.getDashboard({}))
        : Promise.resolve({ label: 'Publicidad', value: null }),
    ]);
    const results = PUBLICIDAD_VISIBLE ? [accounts, partners, glAccounts, audit, ads] : [accounts, partners, glAccounts, audit];
    ...
    <div className="space-y-5">
      ...
      <Resumen
        datos={[
          ...
          { label: 'Business Partners', value: data?.partners ? totalOf(data.partners) : '—' },
          { label: 'Cuentas GL', value: data?.glAccounts ? totalOf(data.glAccounts) : '—' },
      ...
      <div className={PUBLICIDAD_VISIBLE ? 'grid gap-4 grid-cols-1 lg:grid-cols-3' : 'grid gap-4 grid-cols-1 lg:grid-cols-2'}><ModuleCard title="CRM B2B" icon="business_center" href="/operaciones/crm/cuentas" detail={`${totalOf(data?.accounts ?? null)} cuentas disponibles`} links={[['Pipeline','/operaciones/crm/oportunidades'],['Propuestas','/operaciones/crm/propuestas'],['Onboarding','/operaciones/crm/onboarding']]} /><ModuleCard title="Núcleo financiero" icon="account_balance_wallet" href="/operaciones/contabilidad/cuentas-gl" detail={`${totalOf(data?.glAccounts ?? null)} cuentas GL disponibles`} links={[['Business Partners','/operaciones/contabilidad/business-partners'],['Documentos','/operaciones/contabilidad/documentos'],['Cierres','/operaciones/contabilidad/cierres']]} />{!PUBLICIDAD_VISIBLE ? null : <ModuleCard title="Operación publicitaria" icon="campaign" href="/operaciones/ads/dashboard" detail={`${numeric(data?.ads ?? null, 'activeCampaigns')} campañas activas`} links={[['Anunciantes','/operaciones/ads/anunciantes'],['Moderación','/operaciones/ads/moderacion'],['Delivery','/operaciones/ads/delivery-monitor']]} />}</div>
