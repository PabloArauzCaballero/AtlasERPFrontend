...
export function ExecutiveDashboard() {
  ...
  return (
    <div className="space-y-5">
      <WorkspaceHeader eyebrow="Panel de control institucional" title="Executive Overview" description="Visión consolidada de la operación B2B, financiera, publicitaria y de control interno de ATLAS." actions={<><AtlasButton variant="secondary" icon="refresh" onClick={resource.reload}>Actualizar</AtlasButton><AtlasButton variant="secondary" icon="download">Exportar</AtlasButton></>} />
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.6fr)_360px]">
        <Panel title="Operational Footprint" description="Volumen real disponible por módulo; no representa una serie histórica." icon="bar_chart" action={resource.status === 'loading' ? <StatusPill tone="warning">ACTUALIZANDO</StatusPill> : <StatusPill tone="success">SINCRONIZADO</StatusPill>}>
      ...
      <div className="grid gap-4 lg:grid-cols-3"><ModuleCard title="CRM B2B" icon="business_center" href="/operaciones/crm/cuentas" detail={`${totalOf(data?.accounts ?? null)} cuentas disponibles`} links={[['Pipeline','/operaciones/crm/oportunidades'],['Propuestas','/operaciones/crm/propuestas'],['Onboarding','/operaciones/crm/onboarding']]} /><ModuleCard title="Financial Core" icon="account_balance_wallet" href="/operaciones/contabilidad/cuentas-gl" detail={`${totalOf(data?.glAccounts ?? null)} cuentas GL disponibles`} links={[['Business Partners','/operaciones/contabilidad/business-partners'],['Documentos','/operaciones/contabilidad/documentos'],['Cierres','/operaciones/contabilidad/cierres']]} /><ModuleCard title="Ads Operations" icon="campaign" href="/operaciones/ads/dashboard" detail={`${numeric(data?.ads ?? null, 'activeCampaigns')} campañas activas`} links={[['Anunciantes','/operaciones/ads/anunciantes'],['Moderación','/operaciones/ads/moderacion'],['Delivery','/operaciones/ads/delivery-monitor']]} /></div>
...
function ModuleCard({ title, icon, href, detail, links }: { title: string; icon: string; href: string; detail: string; links: string[][] }) { return <Panel title={title} icon={icon} action={<Link href={href} className="text-xs font-bold text-[#031636] hover:underline">Abrir →</Link>}><p className="rounded-md bg-slate-50 p-3 text-xs font-semibold text-slate-700">{detail}</p><div className="mt-3 divide-y divide-slate-100">{links.map(([label, route]) => route && label ? <Link key={route} href={route} className="flex items-center justify-between py-2 text-xs text-slate-600 hover:text-[#031636]"><span>{label}</span><Icon name="chevron_right" className="text-[17px]" /></Link> : null)}</div></Panel>; }
