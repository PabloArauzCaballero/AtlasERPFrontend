...
function ContactList({ contacts }: { contacts: unknown[] }) {
  if (!contacts.length) return <div className="rounded-md border border-dashed border-slate-300 bg-slate-50 px-5 py-8 text-center"><Icon name="contact_page" className="text-[30px] text-slate-300" /><p className="mt-2 text-xs font-bold text-slate-700">Sin contactos asociados</p></div>;
...
function Quick({ href, icon, title, detail }: { href: string; icon: string; title: string; detail: string }) {
  return <Link href={href} className="flex items-center gap-3 rounded-md border border-slate-200 p-3 hover:border-slate-300 hover:bg-slate-50"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-md bg-blue-50 text-[#031636]"><Icon name={icon} className="text-[18px]" /></span><span className="min-w-0 flex-1"><b className="block text-xs">{title}</b><span className="block truncate text-[11px] text-slate-500">{detail}</span></span><Icon name="chevron_right" className="text-[17px] text-slate-400" /></Link>;
}
