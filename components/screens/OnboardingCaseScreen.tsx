...
export function OnboardingCaseScreen() {
  ...
  return (
    <div className="space-y-5">
      <WorkspaceHeader breadcrumbs={[{ label: 'CRM' }, { label: 'Onboarding' }]} title="Onboarding Case Management" description="Coordine requisitos legales, operativos y técnicos antes de activar un comercio en ATLAS." actions={<><AtlasButton variant="secondary" icon="share">Exportar caso</AtlasButton><AtlasButton icon="send" type="submit" form="create-onboarding-form" loading={createMutation.isLoading}>Enviar revisión</AtlasButton></>} />
      ...
      <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1.5fr)_340px]">
        <div className="space-y-4">
          <Panel title="Account Overview" icon="domain"><form id="create-onboarding-form" onSubmit={createCase} className="grid gap-3 md:grid-cols-2"><FormField label="UUID cuenta" name="accountId" required /><FormField label="UUID responsable" name="ownerUserId" required /></form></Panel>
        ...
        <aside className="space-y-4 xl:sticky xl:top-20">
          ...
          <Panel title="Audit Log" icon="history_edu"><div className="space-y-4 text-xs"><AuditRow icon="edit" title="Caso preparado" detail="Los requisitos se validan antes de persistir." /><AuditRow icon="policy" title="Separación de funciones" detail="Legal y Operaciones comparten el flujo." /><AuditRow icon="verified" title="Activación separada" detail="La activación se ejecuta en la vista dedicada." /></div></Panel>
