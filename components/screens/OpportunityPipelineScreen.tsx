...
export function OpportunityPipelineScreen() {
  /* Cuenta y responsable estan catalogados: se eligen. */
  ...
  return (
    <div className="space-y-5">
      ...
      <Panel title="Mover oportunidad" description="Elija una del tablero y su nueva etapa" icon="swap_horiz"><form onSubmit={moveCard} className="grid gap-3 md:grid-cols-[minmax(260px,1fr)_240px_minmax(260px,1fr)_auto]"><FormField kind="select" label="Oportunidad" name="opportunityId" required options={[{ label: cards.length ? '— Elija la oportunidad —' : '— No hay oportunidades —', value: '' }, ...cards.map((card) => ({ value: String(card.id), label: `${String(card.name ?? '')} — ${String(card.stage ?? '')}` }))]} /><FormField kind="select" label="Nueva etapa" name="stage" options={[...stages.map((stage) => ({ label: stageLabels[stage], value: stage })), { label: 'Closed Lost', value: 'CLOSED_LOST' }]} /><FormField label="Motivo de pérdida" name="lossReason" placeholder="Obligatorio para Closed Lost" /><div className="flex items-end"><AtlasButton type="submit" icon="sync_alt" loading={moveMutation.isLoading}>Mover</AtlasButton></div></form>{moveMutation.error ? <div className="mt-3"><InlineNotice tone="danger">{moveMutation.error}</InlineNotice></div> : null}</Panel>
    </div>
  );
}
