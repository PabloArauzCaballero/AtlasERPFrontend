      ...
      <div className="custom-scrollbar overflow-x-auto pb-2">
        <div className="grid min-w-[1400px] grid-cols-6 gap-3">
          {stages.map((stage) => {
            ...
            return <section key={stage} className="rounded-lg border border-slate-200 bg-slate-100/70 p-2"><header className="mb-2 flex items-center justify-between px-1 py-1"><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[#031636]" /><h2 className="text-xs font-bold text-slate-800">{stageLabels[stage]}</h2></div><span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-bold text-slate-500 ring-1 ring-slate-200">{stageCards.length}</span></header><div className="space-y-2">{stageCards.map((card) => <article key={String(card.id)} className="rounded-md border border-slate-200 bg-white p-3 shadow-sm"><div className="flex items-start justify-between gap-2"><p className="text-xs font-bold text-slate-900">{String(card.name ?? 'Oportunidad')}</p><button className="text-slate-400"><Icon name="more_horiz" className="text-[18px]" /></button></div><p className="mt-1 font-mono text-[9px] text-slate-400">{String(card.id ?? '').slice(0, 18)}</p><div className="mt-3 flex items-center justify-between"><span className="text-[11px] font-bold text-slate-700">{formatBob(Number(card.expectedMonthlyVolume ?? 0))}</span><StatusPill tone="info" dot={false}>{String(card.probability ?? 0)}%</StatusPill></div></article>)}{!stageCards.length ? <div className="rounded-md border border-dashed border-slate-300 bg-white/60 px-3 py-8 text-center text-[11px] text-slate-400">Sin oportunidades visibles</div> : null}</div></section>;
          })}
        </div>
      </div>
