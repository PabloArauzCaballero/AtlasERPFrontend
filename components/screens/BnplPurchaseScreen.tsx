...
export function BnplPurchaseScreen() {
  ...
  /*
   * El comercio NO teclea su propio identificador: sale de su sesion. El staff interno si elige a
   * que comercio entra, y eso queda auditado como acceso delegado. Antes ambos tecleaban un uuid,
   * que para el comercio era pedirle un dato que nunca ha visto y para el staff una via de
   * equivocarse de cuenta en silencio.
   */
  const scope = useMerchantScope();
  const branches = useOptions(useCallback(async () => {
    const rows = await portalService.listBranches(scope.accountId);
    return rows.map((row) => ({ value: String(row.id), label: `${String(row.name ?? 'Sucursal')} — ${String(row.city ?? '')}`.trim() }));
  }, [scope.accountId]));
