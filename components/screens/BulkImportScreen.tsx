    ...
    <div className="space-y-5">
      ...
      <div className="grid items-start gap-4 grid-cols-[minmax(0,1fr)] xl:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <aside className="space-y-4 xl:sticky xl:top-20">
          <Panel title="Pre-flight Audit" icon="rule"><div className="space-y-3"><Audit label="Encabezados requeridos" ok={rows.length > 0 && invalidRows === 0} /><Audit label={`Límite de ${props.maxRows} filas`} ok={rows.length <= props.maxRows} /><Audit label="Duplicados internos" ok={rows.length > 0} detail="El sistema repite esta comprobación." /><Audit label="Validación Zod" ok={rows.length > 0} detail="Se ejecuta al enviar el lote." /></div></Panel>
          <Panel title="Propiedades del lote" icon="inventory_2"><label className="block text-xs font-bold text-slate-700">Identificador externo <span className="font-normal text-slate-500">(opcional)</span><input className="mt-1.5 h-9 w-full rounded-md border border-slate-300 px-3 text-xs" value={batchExternalId} onChange={(event) => setBatchExternalId(event.target.value)} /></label><p className="mt-1 text-[10px] text-slate-500">Opcional: sólo si tu sistema ya numera sus lotes; evita importar dos veces el mismo.</p><AtlasButton className="mt-4 w-full" icon="publish" loading={mutation.isLoading} disabled={!validRows.length || invalidRows > 0} onClick={processBatch}>Procesar {validRows.length} registros</AtlasButton></Panel>
