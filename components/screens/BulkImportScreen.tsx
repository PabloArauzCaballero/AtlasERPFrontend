...
function Audit({ label, ok, detail }: { label: string; ok: boolean; detail?: string }) { return <div className="flex items-start gap-2"><Icon name={ok ? 'check_circle' : 'radio_button_unchecked'} className={`text-[18px] ${ok ? 'text-emerald-600' : 'text-slate-300'}`} /><div><p className="text-xs font-semibold text-slate-700">{label}</p>{detail ? <p className="text-[10px] text-slate-500">{detail}</p> : null}</div></div>; }
