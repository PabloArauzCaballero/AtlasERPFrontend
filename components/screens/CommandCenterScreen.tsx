...
export function CommandCenterScreen() {
  ...
  return <div className="space-y-5"><WorkspaceHeader breadcrumbs={[{ label: 'Administración' }, { label: 'Command Center' }]} title="Centro de comando" description="Encuentre pantallas, módulos y operaciones disponibles en el frontend ATLAS." />
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1.5fr)_360px]"><Panel title="Resultados de la búsqueda" description={`${results.length} accesos encontrados`} icon="manage_search"><div className="divide-y divide-slate-100">{results.map((item) => <Link key={item.href} href={item.href} className="flex items-center gap-3 px-2 py-3 hover:bg-slate-50"><div className="grid h-9 w-9 place-items-center rounded bg-slate-100 text-[#006a61]"><Icon name={item.area === 'Portal comercio' ? 'storefront' : 'apps'} className="text-[19px]" /></div><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{item.title}</p><p className="truncate text-[10px] text-slate-500">{item.phase} · {item.backend}</p></div><StatusPill tone={item.status === 'integrada' ? 'success' : item.status === 'solo-accion' ? 'warning' : 'danger'}>{item.status}</StatusPill><Icon name="chevron_right" className="text-[18px] text-slate-400" /></Link>)}</div></Panel><div className="space-y-4"><Panel title="Explorador de módulos" icon="category"><div className="space-y-3">{grouped.map((item) => <div key={item.label} className="flex items-center justify-between rounded bg-slate-50 px-3 py-2.5 text-xs"><span className="font-semibold">{item.label}</span><b>{item.count}</b></div>)}</div></Panel><Panel title="Brecha de backend" icon="lan"><p className="text-xs leading-5 text-slate-600">La búsqueda de registros por NIT, UUID, factura o usuario no se simula. Requiere un contrato global seguro con autorización por módulo.</p></Panel></div></div>
  </div>;
}
