    ...
    <aside className="fixed bottom-0 left-0 top-16 z-40 hidden w-64 flex-col border-r border-slate-200 bg-[#f8fafc] lg:flex">
      <div className="border-b border-slate-200 px-5 py-5">
        <div className="flex items-center gap-3">
          <span className="grid h-10 w-10 place-items-center rounded-md bg-[#031636] text-white"><Icon name="account_balance" className="text-[23px]" /></span>
          <div><p className="text-sm font-extrabold text-[#031636]">Financial Core</p><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">Enterprise Hub</p></div>
        </div>
      </div>
      <nav className="custom-scrollbar flex-1 overflow-y-auto p-3">
        <Link href="/operaciones" className={cn('mb-2 flex items-center gap-3 rounded-md px-3 py-2.5 text-xs font-bold', pathname === '/operaciones' ? 'bg-[#1a2b4c] text-white' : 'text-slate-600 hover:bg-white')}>
          ...
          <section className="mb-3" key={group.label}>
            ...
            <div className="space-y-0.5">
              {group.items.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} className={cn('flex items-center gap-3 rounded-md px-3 py-2 text-xs font-semibold transition-colors', active ? 'bg-[#1a2b4c] text-white shadow-sm' : 'text-slate-600 hover:bg-white hover:text-[#031636]')}>
