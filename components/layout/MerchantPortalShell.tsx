  ...
  return (
    <div className="min-h-screen bg-[#f5f7fb] text-slate-900">
      <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-slate-200 bg-white px-5 shadow-sm">
        <div className="flex items-center gap-3"><div className="grid h-8 w-8 place-items-center rounded bg-[#006a61] text-white"><Icon name="account_balance" className="text-[19px]" /></div><div><p className="text-sm font-extrabold tracking-tight">ATLAS ERP</p><p className="text-[9px] font-bold uppercase tracking-[.18em] text-slate-400">Merchant Portal</p></div></div>
      ...
      <div className="mx-auto grid min-h-[calc(100vh-56px)] max-w-[1600px] lg:grid-cols-[220px_minmax(0,1fr)]">
        <aside className="hidden border-r border-slate-200 bg-white p-4 lg:block">
          ...
          <nav className="space-y-1">{links.map((item) => <Link key={item.href} href={item.href} className="flex items-center gap-3 rounded-md px-3 py-2.5 text-xs font-semibold text-slate-600 transition hover:bg-slate-100 hover:text-[#006a61]"><Icon name={item.icon} className="text-[18px]" />{item.label}</Link>)}</nav>
