...
import { loadAccountingPeriods, loadB2BAccounts, loadB2BContracts, loadBusinessPartners, loadGlAccounts, loadLedgers, loadLegalEntities, loadMerchantInvoices, loadReceivables, withEmpty } from '@/services/optionLoaders';
...
export default function B2BBillingPage() {
  ...
  return <div className="space-y-5"><RecordsPanel title="Facturas del comercio" load={b2bService.listMerchantInvoices} emptyHint="Emite la primera factura con el formulario de abajo." /><MultiActionWorkspace moduleLabel="CRM" title="Nueva factura B2B" description="Emita facturas por MDR u otros conceptos y registre pagos con aplicación explícita a cuentas por cobrar." actions={[
    { id: 'invoice', title: 'Datos de la factura', description: 'Agrupa CxC pendientes en una factura comercial.', icon: 'receipt_long', submitLabel: 'Emitir factura', submitIcon: 'send', onSubmit: issue, fields: [
      ...
      { name: 'receivableIdsCsv', label: 'Cuenta por cobrar a facturar', type: 'select', required: true, span: 2, valueKind: 'stringList', optionsLoader: loadReceivables }, { name: 'externalTaxRef', label: 'Referencia fiscal externa', optional: true, span: 2 },
    ] },
    { id: 'payment', title: 'Registrar pago del comercio', description: 'Registra un pago y su aplicación a una CxC.', icon: 'payments', submitLabel: 'Registrar pago', onSubmit: payment, fields: [
    ...
    { id: 'post-gl', title: 'Postear al mayor (contabilidad)', description: 'Genera el asiento de venta (Debe CxC / Haber Ingreso / Haber IVA) de una factura merchant en atlas_accounting.', icon: 'account_balance', submitLabel: 'Postear al mayor', submitIcon: 'sync_alt', onSubmit: postToGl, fields: [
      { name: 'invoiceId', label: 'Factura del comercio', type: 'select', required: true, span: 2, optionsLoader: loadMerchantInvoices },
