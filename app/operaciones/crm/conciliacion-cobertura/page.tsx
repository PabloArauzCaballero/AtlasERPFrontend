...
import { loadInstallments, loadPayables, loadRecoveries } from '@/services/optionLoaders';
export default function CoverageReconciliationPage() {
  ...
  return <div className="space-y-5"><RecordsPanel title="Coberturas por cobrar" load={b2bService.listPayables} emptyHint="Programa la primera cobertura con el formulario de abajo." /><MultiActionWorkspace moduleLabel="CRM" title="Cobertura y Conciliación" description="Programe coberturas, confirme pagos, aplique recuperaciones y ejecute conciliaciones por período." actions={[
    { id: 'coverage', title: 'Programación de Cobertura', description: 'Genera payable por incumplimiento de una cuota.', icon: 'shield', submitLabel: 'Programar cobertura', onSubmit: b2bService.createPayable, fields: [{ name: 'installmentId', label: 'Cuota en mora', type: 'select', required: true, span: 2, optionsLoader: loadInstallments }, { name: 'scheduledPaymentDate', label: 'Fecha programada', type: 'date', required: true }, { name: 'reason', label: 'Motivo', defaultValue: 'CUSTOMER_INSTALLMENT_DEFAULT_COVERAGE' }] },
    { id: 'paid', title: 'Confirmar Pago', description: 'Marca una cobertura payable como pagada.', icon: 'paid', submitLabel: 'Marcar pagado', onSubmit: markPaid, fields: [{ name: 'payableId', label: 'Cobertura a pagar', type: 'select', required: true, span: 2, optionsLoader: loadPayables }, { name: 'paidAt', label: 'Fecha/hora pago', required: true, span: 2 }] },
    { id: 'recovery', title: 'Aplicar Recuperación', description: 'Aplica pago del consumidor a una recuperación.', icon: 'currency_exchange', submitLabel: 'Aplicar pago', onSubmit: recovery, fields: [{ name: 'recoveryId', label: 'Recuperación abierta', type: 'select', required: true, span: 2, optionsLoader: loadRecoveries }, { name: 'amount', label: 'Monto', type: 'number', valueKind: 'number', required: true, span: 2 }] },
    { id: 'run', title: 'Ejecutar conciliación', description: 'Ejecuta conciliación para un rango de fechas.', icon: 'sync_alt', submitLabel: 'Ejecutar conciliación', submitIcon: 'play_arrow', onSubmit: b2bService.createReconciliationRun, fields: [{ name: 'periodStart', label: 'Desde', type: 'date', required: true }, { name: 'periodEnd', label: 'Hasta', type: 'date', required: true }] },
  ]} /></div>;
}
